...
export const Footer: FC = () => {
  ...
  const menuTwo = [
    "Press Inquiries", "Social Media Support", "Image & B-roll", "Site Map"
  ];
            ...
            <div className="p-10">
              ...
              <form onSubmit={handleSubmit} className="mb-6 space-y-2" >
                ...
                <div className="relative">
                  <input type="email" id="email" name="email" className="bg-default-950/60 pe-40 ps-4 h-12 w-full rounded-lg border-white/10 py-4 text-white backdrop-blur-3xl focus:border-white/10 focus:ring-0" />
                  <button type="submit" disabled={state.submitting} className="hover:bg-primary-hover hover:border-primary-hover border-primary bg-primary end-[6px] absolute top-[6px] inline-flex h-9 items-center justify-center rounded-md px-6 gap-2 text-white transition-all" >
                    Subscribe

                  </button>
                </div>
              </form>
  <h6 className="mb-4 text-base text-white " >
              Follow us
            ...
            <ul className="flex flex-wrap items-center gap-1"  >
              {[<TiSocialFacebook/>, <TiSocialLinkedin/>, <TiSocialTwitter/>, <TiSocialYoutube/> ].map((social, index)=> (
                <li key={index} >
                  <a href="#" className="hover:bg-primary group inline-flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 transition-all duration-500">
      ...
      <div className="md:text-start container flex h-full flex-wrap items-center justify-center gap-4 text-center md:justify-between lg:px-20">
        <p className="text-default-400 text-base font-medium" >
          @SolanaAI - <a href="#">
            Design & Created <i data-lucide="heart" className="inline h-4 w-4 fill-red-500 text-red-500" ></i> by @Kaosiso
          </a>
